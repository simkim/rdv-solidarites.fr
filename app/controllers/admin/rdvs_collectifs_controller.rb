...
class Admin::RdvsCollectifsController < AgentAuthController
  ...
  def index
    @rdvs = policy_scope(Rdv).joins(:motif).where(motifs: { location_type: :collective }).where(organisation: current_organisation)
  ...
  def new
    ...
    @motifs = policy_scope(Motif).available_motifs_for_organisation_and_agent(current_organisation, current_agent).where(location_type: :collective)
