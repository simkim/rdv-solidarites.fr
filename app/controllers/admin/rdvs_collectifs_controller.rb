...
class Admin::RdvsCollectifsController < AgentAuthController
  ...
  def index
    ...
    @has_at_least_one_motif_collectif = policy_scope(Motif).available_motifs_for_organisation_and_agent(current_organisation, current_agent).where(collectif: true).any?
